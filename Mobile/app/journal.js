    ...
    */
        ...
        if (inputText.trim() !== '') {
            try {
                ...
                if (res.data.hasOwnProperty('userExists')) {
                    setTemp(1);
                    router.push('/signin');
                } else if (res.data.hasOwnProperty('authentication')) {
                    setTemp(2);
                    router.push('/signin');
                } else if(res.data.hasOwnProperty('verification')) {
                    setTemp(3);
                    router.push('/signin');
                } else if (res.data.hasOwnProperty('expired')) {
                    setTemp(4);
                    router.push('/signin');
                } else if (res.data.hasOwnProperty('server')) {
                    setTemp(5);
                    router.push('/signin');
                } else {
    ...
    return (
        <View className={`w-[100vw] h-[100vh] flex justify-start items-center`}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className={`${queryFocus ? 'w-[100vw] h-[97vh] pb-[0vh]' : 'flex-1 pb-[3vh]'} flex flex-col items-center justify-between gap-y-[2%] pb-[3vh] pt-[3vh]`} keyboardVerticalOffset={queryFocus ? 0 : 0}>
                <Pressable className='w-[90%] flex flex-row justify-end pt-[6%]' onPress={startSession}>
                ...
                <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={true}>
                    <ScrollView ref={scrollViewRef} className='w-[90%] h-[68vh]' contentContainerClassName={`flex w-[100%] flex-col ${queryFocus ? 'gap-y-[3%]' : 'gap-y-[3%]'} items-center`} onContentSizeChange={() => {scrollViewRef.current?.scrollToEnd({ animated: false })}} showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false}>
                        {typeof sent === "boolean" && !sent ?
                            <View className={`flex w-[100%] h-[63vh] flex-col items-start gap-y-[3%]`}>
                                <View className='w-[80%]'>
                ...
                <View className={`${queryFocus ? 'w-[90%] h-[15vh]' : 'w-[90%] h-[15vh]'} flex flex-col justify-between items-start border rounded-3xl`}>
